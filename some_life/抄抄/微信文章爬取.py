...
class gzh_wz(object):
    def __init__(self):
        ...
        with open('标题.txt', 'r', encoding='utf-8')as r:
            titles = r.read().replace('\ufeff', '')
            self.title_l = titles.split('\n')
        self.cookie = input('cookie：')
        self.pass_ticket = input('pass_ticket:')
        self.url_dict = {
            '揭阳蓝城生活': 'https://mp.weixin.qq.com/cgi-bin/appmsg?action=list_ex&begin=0&count=5&fakeid=MzI0MTA2MDMwMA'
                      '==&type=9&query=&token=abcd&lang=zh_CN&f=json&ajax=1',
            '揭东生活圈': 'https://mp.weixin.qq.com/cgi-bin/appmsg?action=list_ex&begin=0&count=5&fakeid=MzI0MzA1ODc0NQ'
                     '==&type=9&query=&token=abcd&lang=zh_CN&f=json&ajax=1 '
        }
        self.headers = {
            'cookie': self.cookie,
            ...
            'referer': 'https://mp.weixin.qq.com/cgi-bin/appmsg?t=media/appmsg_edit_v2&action=edit&isNew=1&type=10'
                       '&token=370616041&lang=zh_CN',
        }
        self.app_url = "http://mp.weixin.qq.com/mp/getappmsgext"
        phoneCookie = "rewardsn=; wxtokenkey=777; wxuin=1797234910; devicetype=Windows10x64; version=62090529; " \
                      "lang=zh_CN; pass_ticket=hjymD+nRjMwS6tz25jYr1rByJW9Yzu8L6Y+I6VDM8EK8bM7Ltcee1dTvMsr6A2I1; " \
                      "wap_sid2=CN7B" \
                      "/tgGElxnWXktSExLc0EyTVFVNGJMTjJ5eVhwSWNDQk5tRHhSd21NaGZNMUoxaWJRdzBlcUJxZG1XUEtuQ1JTekt4dXctS3" \
                      "hLaVFWU0xqdWQxNVJleFAxTGZ4akVFQUFBfjDi67T5BTgNQAE= "
        self.app_headers = {
            "Cookie": phoneCookie,
    ...
    def get_response(self, detail):
        ...
        if self.title in self.title_l:
            ...
            with open('标题.txt', 'a', encoding='utf-8')as w:
                w.write('\n')
                w.write(self.title)
